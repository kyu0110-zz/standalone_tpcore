...
! !INTERFACE:
!
  SUBROUTINE WRITE_wc( ncFile, YYYY, MM, DD, h, m, s, wc )
!
! !USES:
!
  USE CMN_SIZE_MOD
...
!
  ...
  Arr3D = wc
  CALL NC_VAR_DEF( fId, lonId, latId, levId, -1, & 
                   'wc', 'wc', 'kg m^-2 s^-2', 4, VarCt )
  CALL NC_VAR_WRITE( fId, 'wc', Arr3D=Arr3D)  
  DEALLOCATE( Arr3D )
...
! !INTERFACE:
!
  SUBROUTINE WRITE_KZ( ncFile, YYYY, MM, DD, h, m, s, KZ, c_bar )
!
! !USES:
!
  USE CMN_SIZE_MOD
...
!
  ...
  Arr3D = Kz
  CALL NC_VAR_DEF( fId, lonId, latId, levId, -1, & 
                   'Kz', 'Kz', 'Pa^2 s^-1', 4, VarCt )
  CALL NC_VAR_WRITE( fId, 'Kz', Arr3D=Arr3D)  
  DEALLOCATE( Arr3D )
  ...
  Arr3D = c_bar
  CALL NC_VAR_DEF( fId, lonId, latId, levId, -1, & 
                   'c_bar', 'c_bar', 'v/v', 4, VarCt )
  CALL NC_VAR_WRITE( fId, 'c_bar', Arr3D=Arr3D)  
  DEALLOCATE( Arr3D )
