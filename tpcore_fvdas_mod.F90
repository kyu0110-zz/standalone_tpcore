...
!$OMP PRIVATE( IK )
  ...
  ! ====================
    ...
    if (j1p /= 1+1) then
       ...
       !  ========================
               (dap(ik), dbk(ik), area_m2, ps1, q(:,:,ik), &
               1, jm, im, &
               1, im, 1, jm, 1, im, 1, jm)
...
!--------------------------------------------------------
       ...
       !.sds.. convert to "mass"
       dq1(:,:,ik) = q(:,:,ik) * delp1(:,:,ik)
       

     ! ===========================
       call Calc_Advec_Cross_Terms  &
     ! ===========================
            (jn(ik), js(ik), q(:,:,ik), qqu, qqv, &
            ua(:,:,ik), va(:,:,ik), &
